...
-- Safe to run on an existing project. Does not delete operational data.

-- Vendor master/RLS repair for projects where migration 005 failed.
...
set name=excluded.name,worker_id=excluded.worker_id,status=excluded.status,updated_at=now();
...
on public.vendor_master for select to authenticated using (true);
